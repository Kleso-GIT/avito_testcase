...
async def get_user_info(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
        inventory_result = await db.execute(
            select(Inventory).filter_by(owner_id=current_user.id)
        )
        ...
        return {
            ...
            "coinHistory": {
                "received": [
                    {"fromUser": t.sender.username if t.sender else "System", "amount": t.amount}
                    for t in received_transactions
                ],
                "sent": [
                    {"toUser": t.receiver.username if t.receiver else "System", "amount": t.amount}
